...
const faqs = [
  ...
  },
  {
    ...
    answer:
      "Yes! Contact us via Viber or WhatsApp for wholesale inquiries. We offer special pricing for bulk orders and resellers interested in our products.",
  },
]
...
export function FAQSection() {
  ...
  return (
    <section id="faq" className="py-16 sm:py-24 bg-card px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto">
        <div className="text-center mb-12">
          <h2 className="text-4xl sm:text-5xl font-bold text-foreground mb-4 text-balance">
            Frequently Asked <span className="text-primary">Questions</span>
          </h2>
          <p className="text-lg text-foreground/70">Everything you need to know about our products and service</p>
        </div>
          ...
          {faqs.map((faq) => (
            <div key={faq.id} className="rounded-lg border border-border/50 bg-background overflow-hidden">
              <button
                ...
                className="w-full px-6 py-4 sm:px-8 sm:py-5 flex items-center justify-between hover:bg-card transition-colors"
              >
                <span className="text-lg font-semibold text-foreground text-left pr-4">{faq.question}</span>
                <ChevronDown
              ...
              {/* Expanded Answer */}
              {expandedId === faq.id && (
                <div className="px-6 sm:px-8 py-4 sm:py-5 bg-card border-t border-border/50">
        ...
        <div className="mt-16 p-8 rounded-lg bg-primary/10 border border-primary/20 text-center">
          ...
          <p className="text-foreground/70 mb-6">
            Our customer service team is ready to help. Contact us on Viber, WhatsApp, or email.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a
              href="#"
              className="px-6 py-3 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 font-semibold inline-block"
            >
              Viber/WhatsApp
            </a>
            <a
              href="mailto:support@glowvital.ph"
              className="px-6 py-3 rounded-lg border border-primary/30 text-foreground hover:bg-primary/5 font-semibold inline-block"
            >
              Email Support
            </a>
          </div>
